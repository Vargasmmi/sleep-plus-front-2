...
const getActivityResource = (resource) => {
    const resourceMap = {
...
export const customDataProvider = {
    ...
    update: async ({ resource, id, variables }) => {
        ...
        // Log update activity
        activityLogService.logUpdate(getActivityResource(resource), id.toString(), variables, previousData);
        return {
            data: response.data,
        };
    },
    ...
    createMany: async ({ resource, variables }) => {
        const response = await Promise.all(variables.map((item) => axiosInstance.post(`/${resource}`, item)));
    ...
    deleteMany: async ({ resource, ids }) => {
        const response = await Promise.all(ids.map((id) => axiosInstance.delete(`/${resource}/${id}`)));
    ...
    updateMany: async ({ resource, ids, variables }) => {
        const response = await Promise.all(ids.map((id) => axiosInstance.put(`/${resource}/${id}`, variables)));
