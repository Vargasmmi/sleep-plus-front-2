...
export const ThemedLayoutWithDynamicMenu = ({ children }) => {
    ...
    useEffect(() => {
        const checkResourceAccess = async () => {
            ...
            const checkedResources = await Promise.all(resources.map(async (resource) => {
                ...
                try {
                    ...
                    if (overrides.length > 0) {
                        ...
                        const listOverride = userOverride.permissions.find((p) => p.resource === resource.name && p.action === "list");
                        const viewOverride = userOverride.permissions.find((p) => p.resource === resource.name && p.action === "view");
                        if (listOverride) {
                            hasOverride = true;
    ...
    const CustomSider = () => {
        return (_jsx(ThemedSiderV2, { render: ({ items, logout, collapsed }) => {
                ...
                return (_jsxs(_Fragment, { children: [filteredItems, logout] }));
            } }));
    };
    // Pass filtered resources to the layout context
    if (isLoading) {
        return (_jsx(ThemedLayoutV2, { children: children }));
    }
    return (_jsx(ThemedLayoutV2, { Sider: CustomSider, children: children }));
};
